...
- **Erwähnungs-Benachrichtigung nennt fremde Server nur generisch (Phase 9):** Für Kanäle des gerade NICHT ausgewählten Servers fehlt dem Client das Kanal-Mapping – die Notification sagt dann nur „… erwähnt dich in einem Kanal“. Fix zusammen mit Phase 12 (Benachrichtigungen) sinnvoll
...
- **Reconnect-Rejoin setzt Mute/Deafen kurz zurück (Phase 10):** Nach einem Gateway-Reconnect registriert der Client die Voice-Session neu (`POST …/join`, das serverseitig auf ungemutet setzt) und zieht den lokalen Mute-/Deafen-Zustand direkt per PATCH nach – für andere kann der Zustand einen Moment falsch aussehen
